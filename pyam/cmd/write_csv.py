...
def main(args=None):
    ...
    def get_value(student_id):
        ...
        cohort.log.warning("No spreadsheet for %s found", student_id)
        return None
    ...
    for student in marks.keys():
        cohort.log.warning("No CSV mark recorded for student %s",
                           student.name())
...
def get_id_and_mark(path):
    ...
    student_id = None
    mark = None
    for title, coord in wb.defined_names["student_id"].destinations:
        student_id = wb[title][coord]
    for title, coord in wb.defined_names["mark"].destinations:
        mark = wb[title][coord]
